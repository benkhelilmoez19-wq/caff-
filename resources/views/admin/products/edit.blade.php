...
@section('title', 'Modifier un Produit | Caffée Rajel Kbir')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-coffee-100 shadow-sm overflow-hidden">
        <div class="p-6 border-b border-coffee-100 bg-coffee-50/30">
            <h2 class="font-serif font-bold text-xl text-coffee-900">Modifier le produit : {{ $product->name }}</h2>
            <p class="text-xs text-stone-500 mt-1">Mettez à jour les informations ci-dessous. Si vous ne choisissez pas de nouvelle image, l'ancienne sera conservée.</p>
        </div>
        ...
        {{-- FORMULAIRE MODIFIÉ : Route update avec l'ID du produit et la méthode PUT --}}
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="space-y-3 flex flex-col items-center lg:items-start">
                    ...
                    <div class="relative w-full aspect-square max-w-[260px] bg-stone-50 rounded-2xl border-2 border-dashed border-stone-200 hover:border-coffee-600/40 transition flex flex-col items-center justify-center overflow-hidden p-2 group">
                        ...
                        {{-- GESTION DE L'IMAGE ACTUELLE : Si le produit a déjà une image, on l'affiche par défaut --}}
                        @if($product->image)
                            <img id="image-preview" src="{{ Storage::url(str_replace('\\', '/', $product->image)) }}" alt="Aperçu" class="w-full h-full object-cover rounded-xl">
                            <div id="preview-placeholder" class="hidden text-center p-4 space-y-2 pointer-events-none">
                        @else
                        ...
                        <button type="button" id="clear-image" class="{{ $product->image ? '' : 'hidden' }} absolute top-3 right-3 bg-white/90 hover:bg-white text-stone-700 hover:text-brick-600 shadow-sm p-1.5 rounded-full transition cursor-pointer border-0">
                ...
                <div class="lg:col-span-2 space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" placeholder="Ex: Capucin, Thé aux pignons..." required
                    ...
                    <div class="space-y-1.5 w-full md:w-1/2">
                        ...
                        <div class="relative">
                            {{-- Modifié pour afficher la valeur existante --}}
                            <input type="number" step="0.001" name="price" id="price" value="{{ old('price', $product->price) }}" placeholder="0.000" required
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea name="description" id="description" rows="4" placeholder="Décrivez votre produit..."
                            class="w-full px-4 py-2.5 rounded-xl border border-stone-200 focus:border-coffee-600 focus:ring-2 focus:ring-coffee-600/10 outline-none transition text-sm resize-none bg-stone-50/30">{{ old('description', $product->description) }}</textarea>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        clearBtn.addEventListener('click', function(e) {
            e.preventDefault();
            imageInput.value = ''; // Vide la sélection actuelle
            
            // Si le produit avait déjà une image à l'origine, on la remet
            @if($product->image)
                imagePreview.setAttribute('src', "{{ Storage::url(str_replace('\\', '/', $product->image)) }}");
                imagePreview.classList.remove('hidden');
                placeholder.classList.add('hidden');
                clearBtn.classList.add('hidden'); // cache le bouton après reset
            @else
